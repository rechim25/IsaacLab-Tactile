...
def preprocess_observation(observations: dict[str, np.ndarray]) -> dict[str, Tensor]:
    ...
    # =========================================================================
    # Handle IsaacLab Tactile Remote format: rgb_table, rgb_wrist, tactile, poses
    # =========================================================================
    
    # Process rgb_table and rgb_wrist images
    for img_key in ["rgb_table", "rgb_wrist"]:
        if img_key in observations:
            ...
            if img_tensor.ndim == 4:
                b, d1, d2, d3 = img_tensor.shape
                # If channel-last (B, H, W, C) with C in {1,3,4}
                if d3 in (1, 3, 4) and d3 < d1 and d3 < d2:
                    # Convert to channel-first
                    img_tensor = einops.rearrange(img_tensor, "b h w c -> b c h w").contiguous()
                # else assume already channel-first
            
            # Convert uint8 to float32 [0,1]
            if img_tensor.dtype == torch.uint8:
                img_tensor = img_tensor.float() / 255.0
            elif img_tensor.dtype != torch.float32:
                img_tensor = img_tensor.float()
            
            return_observations[f"{OBS_IMAGES}.{img_key}"] = img_tensor
    ...
    for key in ["eef_pos", "eef_quat", "base_pos", "base_quat", "gripper_qpos"]:
        if key in observations:
            ...
            # Use observation. prefix so it's included in the transition's observation dict
            return_observations[f"{OBS_STR}.{key}"] = val
    
    # Pass through tactile force grid
    if "tactile_force_grid" in observations:
        tac = observations["tactile_force_grid"]
        if isinstance(tac, np.ndarray):
            tac = torch.from_numpy(tac).float()
        elif isinstance(tac, torch.Tensor):
            tac = tac.float()
        
        # Add batch dim if needed (expected: B, N, H, W, 3)
        if tac.ndim == 4:  # (N, H, W, 3)
            tac = tac.unsqueeze(0)
        
        # Use observation. prefix
        return_observations[f"{OBS_STR}.tactile_force_grid"] = tac
...
def add_envs_task(env: gym.vector.VectorEnv, observation: RobotObservation) -> RobotObservation:
    ...
    if hasattr(env.envs[0], "task_description"):
        ...
        # Safely determine num_envs: prefer from observation, fall back to env.num_envs
        if observation:
            first_key = list(observation.keys())[0]
            first_val = observation[first_key]
            if hasattr(first_val, "shape") and len(first_val.shape) > 0:
                num_envs = first_val.shape[0]
            else:
