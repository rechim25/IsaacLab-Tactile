...
@EnvConfig.register_subclass("isaaclab_tactile")
@dataclass
class IsaacLabTactileEnv(EnvConfig):
    """
    Configuration for IsaacLab environments with tactile sensing integration.

    This config is designed for SmolVLA training/evaluation with the canonical
    IsaacLab tactile policy convention:
    ...
    # Image dimensions
    # Must match the training dataset / policy input feature shapes.
    # Our converted IsaacLab tactile datasets store images as (3, 224, 224).
    ...
    tactile_force_grid_shape: tuple[int, ...] = (2, 10, 12, 3)  # (N_fingertips, H, W, 3)
    tactile_resultant_force_shape: tuple[int, ...] | None = None  # Optional: (N_fingertips, 3)

    # Raw observation keys (for processor to find data)
    ...
    def __post_init__(self):
        # Set action feature (7D)
        self.features[ACTION] = PolicyFeature(type=FeatureType.ACTION, shape=(7,))
        self.features_map[ACTION] = ACTION

        # Set state feature (11D)
        self.features[OBS_STATE] = PolicyFeature(type=FeatureType.STATE, shape=(11,))
        self.features_map[OBS_STATE] = OBS_STATE

        # Add camera features
        if self.camera_keys:
            for i, cam_key in enumerate(self.camera_keys.split(",")):
                cam_key = cam_key.strip()
                if cam_key:
        ...
        self.features[tactile_key] = PolicyFeature(
            # IMPORTANT: mark as TACTILE so the policy's normalization_mapping applies TACTILE rules.
...
@EnvConfig.register_subclass("isaaclab_tactile_remote")
@dataclass
class IsaacLabTactileRemoteEnv(EnvConfig):
    """
    Configuration for remote IsaacLab tactile environments (server mode).

    Used when IsaacLab runs as a separate process/server and communicates
    with LeRobot over a network protocol (e.g., ZeroMQ).

    The server returns raw observations and the client applies env pre/post
    processors to convert to/from the canonical policy convention.
    """
