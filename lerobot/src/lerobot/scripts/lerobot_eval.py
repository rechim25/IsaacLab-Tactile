...
def rollout(
    ...
    while not np.all(done) and step < max_steps:
        ...
        # Cache base_quat for action postprocessor (needed for base-to-world frame transform)
        # This must be done BEFORE env_preprocessor consumes it
        cached_base_quat = observation.get(f"{OBS_STR}.base_quat")
        ...
        # Build action transition with base_quat for frame transform
        action_transition = {ACTION: action}
        if cached_base_quat is not None:
            action_transition[TransitionKey.COMPLEMENTARY_DATA] = {"base_quat_w": cached_base_quat}
